...
/**
 * 将各个表中需要的数据联合成一张大表，插入hive库中，可以按天插入或者按时间段插入,根据dayattendance末位组织代码插入组织
 */
...
class insertIntoHrBigTableEmpHistory(spark: SparkSession) {
  def logic(): Unit = {
...
//    attendance.limit(10).show()
    attendance.cache()
    val baseTable = attendance.repartition(10)
      ...
      .join(dept,attendance("value30")===dept("deptcode"),"left")
      .join(sys_company,dept("deptcode")===sys_company("fnumber"),"left")
      .filter("empnum is not null") //为了过滤当天新增加的人员（默认不考虑）
      .select("emp.empnum","emp.emprank","emp.empname","emp.isleave","emp.atttype","record.shiftcode","dayWork.shiftname",
